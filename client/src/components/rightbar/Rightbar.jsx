...
export default function Rightbar({ user }) {
    ...
    const handleClick = async() => {
        try{
            if(followed) {
                await axios.put(`/users/${user.id}/unfollow`, {userId: currentUser._id})
                dispatch({type: "UNFOLLOW", payload:user._id})
            }else{
                await axios.put(`/users/${user.id}/follow`, {userId: currentUser._id})
        ...
        return (
            ...
            {user.username !== currentUser.username && (
                ...
                <div className="rightbarFollowings">
                    {friends.map((friend) => (
                        <NavLink key={friend._id} to={`/profile/${friend.username}`} className="link">
